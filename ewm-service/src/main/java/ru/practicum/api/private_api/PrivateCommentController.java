package ru.practicum.api.private_api;
...
public class PrivateCommentController {
    ...
    public ResponseEntity<CommentDtoResponse> editCommentByUser(@PathVariable @Min(1) Long userId,
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(commentService.editCommentByUser(userId, commentId, updateComment));
    }

    @DeleteMapping("{userId}/comments/{commentId}")
    public ResponseEntity<Void> removeCommentByUser(@PathVariable @Min(1) Long userId,
                                                  @PathVariable @Min(1) Long commentId) {
    ...
    public ResponseEntity<CommentDtoResponse> viewComment(@PathVariable @Min(1) Long userId,
                                                                @PathVariable @Min(1) Long commentId) {
        log.info("get private comment with userId={} and commentId={}", userId, commentId);
        return ResponseEntity.status(HttpStatus.OK).body(commentService.viewComment(userId, commentId));
    }

    @GetMapping("{userId}/events/{eventId}/comments")
    public ResponseEntity<CommentDtoList> listComments(@PathVariable @Min(1) Long userId,
                                                             @PathVariable @Min(1) Long eventId) {
        log.info("get comments with userId={} and eventId={}", userId, eventId);
        return ResponseEntity.status(HttpStatus.OK).body(commentService.listComments(userId, eventId));
    }
}
